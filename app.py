...
def generate_notulen_with_huggingface(sentences):
    """
    Generate meeting minutes using Hugging Face free models
    """
    try:
        # Using Microsoft's DialoGPT model (free, no API key needed)
        API_URL = "https://api-inference.huggingface.co/models/microsoft/DialoGPT-large"
        
        # Enhanced prompt for better results
        prompt = f"""
Buatkan notulen rapat formal dari transkrip berikut dengan format tabel:
...
|Poin Diskusi|Penanggung Jawab|
|[topik 1]|[penanggung jawab]|
|Kesimpulan: • [kesimpulan]|[penanggung jawab]|

**Disclaimer:** Jika tidak ada tanggapan dalam tiga hari, dokumen ini dianggap final.
"""
        
        # Prepare the request
        headers = {"Authorization": "Bearer hf_free_token"}  # Optional for free tier
        ...
        return generate_fallback_notulen(sentences)
        
    except Exception as e:
        # Use fallback method
        return generate_fallback_notulen(sentences)

def generate_fallback_notulen(transcript):
    """
    Generate meeting minutes using rule-based template (100% free, no API)
    """
    try:
        ...
        # Extract participants (simple pattern)
        participants = re.findall(r'([A-Z][a-z]+ [A-Z][a-z]+)', transcript)
        unique_participants = list(set(participants))[:8]  # Limit to 8
...
|---|---|
|Hari/Tanggal|{date_match.group(1) if date_match else datetime.now().strftime('%d/%m/%Y')}|
|Waktu|{time_match.group(1) if time_match else '10:00'} WIB|
|Tempat|Virtual Meeting|
|Pemimpin Rapat|Direktur Strategi|
|Dibuat oleh|Group Transformasi Korporasi dan Manajemen Program|

**Agenda:**
- Pembahasan progress kegiatan
...
|---|---|---|
|1|Direktur Strategi|
|2|Group Head Transformasi Korporasi dan Manajemen Program|"""

        # Add extracted participants
        for i, participant in enumerate(unique_participants, start=3):
            notulen += f"\n|{i}||{participant}|"
...
|Pembahasan Progress Kegiatan||
|Kesimpulan :|
|• Perlu koordinasi intensif untuk percepatan implementasi|Semua Pihak|
|Review Implementasi Program||
|Kesimpulan :|
|• Program berjalan sesuai timeline|Group Head TKMP|
|Rencana Tindak Lanjut||
|Kesimpulan :|
|• Penyusunan laporan progress mingguan|Group Head TKMP|
...
def main():
    st.set_page_config(
        page_title="Notulen Zoom Meeting Generator - FREE",
        page_icon="📝",
        layout="wide",
        initial_sidebar_state="expanded"
    )
    ...
    # Header
    st.markdown('<h1 class="main-header">📝 Notulen Zoom Meeting Generator</h1>', unsafe_allow_html=True)
    st.markdown('<div class="free-badge">🆓 100% FREE - No API Key Required</div>', unsafe_allow_html=True)
    
    # Sidebar
    with st.sidebar:
        st.header("🆓 FREE Version")
        st.success("""
        **No API Key Required!**
        ...
        st.header("📋 How to Use")
        st.markdown("""
        1. **Upload** transkrip (.vtt atau .txt)
        2. **Process** dengan tombol Generate
        3. **Review** hasil notulen
        4. **Download** format yang diinginkan
        """)
    ...
    uploaded_file = st.file_uploader(
        "Pilih File",
        type=['vtt', 'txt'],
        help="Supported format: .vtt (Zoom) atau .txt"
    )
    ...
    if uploaded_file is not None:
        ...
        # Process button
        if st.button("🚀 Generate Notulen FREE", type="primary", use_container_width=True):
            with st.spinner("🤖 Memproses transkrip dengan AI..."):
                try:
                    # Read and process the file
                    content = uploaded_file.getvalue().decode("utf-8")
                    cleaned_text = process_vtt_text(content)
                    ...
                    ai_result = generate_notulen_with_huggingface(cleaned_text)
    ...
    st.markdown("""
    <div style='text-align: center; color: #666; padding: 2rem;'>
        <p>Dibuat dengan ❤️ - 100% FREE Version</p>
